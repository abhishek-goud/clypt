...
const logHeader = (action, target) => {
  console.log(chalk.magenta(`[CLYPT] ${new Date().toLocaleString("en-GB", { dateStyle: "short", timeStyle: "medium" })}`));
  console.log(chalk.white(`→ ${action}: ${target}`));
};

program
  .command("upload <files...>")
  .description("Upload multiple files by appending locations spearated by space and retrieve unique code")
  .action(async (files) => {
    logHeader("Processing files", files.join(", "));
    
    // Prepare files for upload
    const formData = new FormData();
    files.forEach(file => formData.append("files", fs.createReadStream(file)));
    
    let uploadInterval, i = 0;
    ...
    try {
      ...
      const response = await axios.post(ENDPOINT_URL, formData, { headers: formData.getHeaders() });
      
      clearInterval(uploadInterval);
      clearSpinner();

      console.log(chalk.green(`Upload completed successfully`));
      console.log(chalk.white(`Code: `) + chalk.bold.cyan(response.data.uniqueCode));
...
program
  ...
  .action(async (code) => {
    ...
    let validationInterval, downloadInterval, i = 0, j = 0;
    ...
    try {
      ...
      const response = await axios({ url: `${ENDPOINT_URL}?code=${code}`, method: "GET", responseType: "stream" });
      ...
      downloadInterval = setInterval(() => {
        process.stdout.write(`\r${chalk.hex("#3b82f6")(spinner[j++ % spinner.length])} Downloading zip file...`);
      }, 80);
      ...
      writer.on("finish", async () => {
        ...
        try {
          const fileType = await axios.get(`${ENDPOINT_URL}/filetype?code=${code}`);
          if (fileType.data?.length > 0) console.log(chalk.bold.cyan(`Files Type: ${fileType.data}`));
        } catch {}
